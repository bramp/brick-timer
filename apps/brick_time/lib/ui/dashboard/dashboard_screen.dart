...
import 'package:brick_time/ui/dashboard/sync_status_widget.dart';
...
class DashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('BrickTime Dashboard'),
        actions: const [
          SyncStatusWidget(),
          SizedBox(width: 16),
        ],
      ...
      body: sessionsAsync.when(
        data: (sessions) {
          ...
          final activeSessions =
              sessions.where((s) => !s.session.isCompleted).toList();
          final historicSessions =
              sessions.where((s) => s.session.isCompleted).toList();
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          // TODO(bramp): Navigate to Rebrickable Search Portal
        },
